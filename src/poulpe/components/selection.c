...
enum poulpe_error poulpe_selection_draw(struct poulpe_selection *selection)
{
    ...
    for (uint32_t i = selection->ajusted.start_line_index; i <= selection->ajusted.end_line_index; i++)
    {
        ...
        {
            float start_size = poulpe_textbuffer_line_subset_textsize(selection->textedit->textbuffer, i, 0, selection->ajusted.start_glyph_index);
            float end_size = poulpe_textbuffer_line_subset_textsize(selection->textedit->textbuffer, i, 0, selection->ajusted.end_glyph_index);
        ...
        {
            float full_size = poulpe_textbuffer_line_full_textsize(selection->textedit->textbuffer, i);
            float subset_size = poulpe_textbuffer_line_subset_textsize(selection->textedit->textbuffer, i, 0, selection->ajusted.start_glyph_index);
        ...
        {
            float text_size = poulpe_textbuffer_line_subset_textsize(selection->textedit->textbuffer, i, 0, selection->ajusted.end_glyph_index);
        ...
        {
            float text_size = poulpe_textbuffer_line_full_textsize(selection->textedit->textbuffer, i);
            text_size = fmax(text_size, space_size.x);
            upper_left = (ImVec2) {origin_screen_position.x, origin_screen_position.y + i * igGetTextLineHeight()};
            lower_right = (ImVec2) {origin_screen_position.x + text_size, origin_screen_position.y + (i + 1) * igGetTextLineHeight() };
        }
    ...
    while (i <= selection->ajusted.end_line_index)
    {
        if (selection->ajusted.start_line_index == selection->ajusted.end_line_index)
        {
            for (uint32_t j = selection->ajusted.start_glyph_index; j < selection->ajusted.end_glyph_index; j++)
                poulpe_textbuffer_line_erase(selection->textedit->textbuffer, line, selection->ajusted.start_glyph_index);
            line++;
        }
        else if (i == selection->ajusted.start_line_index)
        {
            uint32_t line_size = poulpe_textbuffer_line_size(selection->textedit->textbuffer, line);
        ...
        else if (i == selection->ajusted.end_line_index)
        {
            for (uint32_t j = 0; j < selection->ajusted.end_glyph_index; j++)
                poulpe_textbuffer_line_erase(selection->textedit->textbuffer, line, 0);
            line++;
        }
        else
        {
            poulpe_textbuffer_text_erase(selection->textedit->textbuffer, line);
        }
        i++;
    }

    if (selection->ajusted.start_line_index < poulpe_textbuffer_text_size(selection->textedit->textbuffer) - 1)
    {
        for (uint32_t i = 0; i < poulpe_textbuffer_line_size(selection->textedit->textbuffer, selection->ajusted.start_line_index + 1); i++)
        {
            struct poulpe_glyph *glyph = poulpe_textbuffer_line_at(selection->textedit->textbuffer, selection->ajusted.start_line_index + 1, i);
            poulpe_textbuffer_line_push_back(selection->textedit->textbuffer, selection->ajusted.start_line_index, glyph);
        }
        poulpe_textbuffer_text_erase(selection->textedit->textbuffer, selection->ajusted.start_line_index + 1);
    }
