...
enum poulpe_error poulpe_linenumber_draw(struct poulpe_linenumber *linenumber)
{
    ...
    if (!igBeginChild_Str("Poulpe##linenumber", (ImVec2) {max_line_number_size, 0.0f}, false, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar))
        goto end_child;
    ...
    {
        ImVec2 size = {max_line_number_size, poulpe_textbuffer_text_size(linenumber->textedit->textbuffer) * igGetTextLineHeight()};
